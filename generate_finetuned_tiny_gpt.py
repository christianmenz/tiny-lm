...
from train_tiny_gpt import TinyGPT, SimpleTokenizer, get_checkpoint_config, load_checkpoint_for_state_dict
...
def load_model_and_tokenizer(model_path: str, tokenizer_path: str, device: torch.device):
    ...
    cfg = get_checkpoint_config(ckpt) or {}
    model = TinyGPT(
    ...
    model.load_state_dict(load_checkpoint_for_state_dict(ckpt), strict=True)
    model.eval()
    return model, tok
...
def generate_once(model, tok, device, prompt, max_new_tokens, temperature, top_k):
    ...
    out = model.generate(ids, max_new_tokens=max_new_tokens, top_k=top_k, temperature=temperature)[0].tolist()
...
def main(argv=None):
    ...
    model, tok = load_model_and_tokenizer(args.model, args.tokenizer, device)
        ...
        try:
            while True:
                ...
                decoded = generate_once(
                    model, tok, device, full_prompt, args.max_new_tokens, args.temperature, args.top_k
                )
    ...
    decoded = generate_once(model, tok, device, full_prompt, args.max_new_tokens, args.temperature, args.top_k)
    if args.chat and args.answer_only:
        print(extract_answer(decoded, args.assistant_tag, args.user_tag))
    else:
        print(decoded)
